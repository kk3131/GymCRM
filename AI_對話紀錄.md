...
### Prompt 18 — 管理者儀表板（dashboard_page.py）

**新增檔案：**
- `dashboard_page.py`：管理者儀表板（manager 限定）

**修改檔案：**
- `auth.py`：PAGE_ACCESS 新增「管理者儀表板」（manager 限定，排第一位，登入後預設進入此頁）
- `app.py`：`import dashboard_page`，`render_page` 加入對應分支

**功能：**

KPI 卡（頁面頂部三格）：
- **今日到館人數**：`SELECT COUNT(*) FROM check_ins WHERE date(check_in_at) = 今天`（台北時區）
- **本月新增會員**：`SELECT COUNT(*) FROM members WHERE strftime('%Y-%m', join_date) = 本月`
- **本月總收入**：`SELECT SUM(amount) FROM payments WHERE strftime('%Y-%m', payment_date) = 本月`，顯示格式 `NT$ x,xxx`

RFM 分群分布（環狀圖）：
- 直接呼叫 `compute_rfm_rows()`，統計各分群人數，與 RFM 分析頁共用同一套分群邏輯
- 顏色：藍（核心）/ 綠（穩定）/ 灰（一般）/ 紅（流失風險）
- hover 顯示人數與百分比
- 圖下附各分群小卡：人數 + 佔比

**設計重點：**
- 分群定義完全重用 `rfm_page.compute_rfm_rows()` 與 `SEGMENT_ORDER`，不重複定義評分邏輯
